...
//  ラッパー関数
// ==================================================
function createStudentUI() { createMatrixSheet('student_list', 'UI_Student_Input', '生徒名'); }
function saveStudentData() { saveMatrixData('UI_Student_Input', 'student_availability', 'student_id'); }
function createTeacherUI() { createMatrixSheet('teacher_list', 'UI_Teacher_Input', '講師名'); }
function saveTeacherData() { saveMatrixData('UI_Teacher_Input', 'teacher_availability', 'teacher_id'); }
...
function createMatrixSheet(listSheetName, uiSheetName, labelName) {
  ...
  const sheetSlots = ss.getSheetByName('lesson_slot');
  const sheetList = ss.getSheetByName(listSheetName);
  const sheetTimeRange = ss.getSheetByName('time_range'); // ★追加

  if (!sheetSlots || !sheetList) {
    Browser.msgBox(`エラー: シート '${listSheetName}' または 'lesson_slot' が見つかりません。`);
    return;
  }
  ...
  // 科目名マッピング
  const sheetCourse = ss.getSheetByName('course');
  let cMap = new Map();
  ...
  if (uiSheetName === 'UI_Student_Input') {
    const sheetReq = ss.getSheetByName('student_course');
  ...
  // 講師用 (UI_Teacher_Input)
    ...
    const sheetTeachable = ss.getSheetByName('teachable_courses');
...
function visualizeSchedule() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheetAllocated = ss.getSheetByName('output_allocated_lessons');
  const sheetSlots = ss.getSheetByName('lesson_slot');
  const sheetStudents = ss.getSheetByName('student_list');
  const sheetTimeRange = ss.getSheetByName('time_range'); // ★追加
