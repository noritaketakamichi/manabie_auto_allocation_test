...
最適化エンジンは**基本制約**と**追加制約**の2種類のハード制約に加え、**ソフト制約**（目的関数へのペナルティ/ボーナス）を使います。すべてのハード制約を守った上で、配置数（コマ数）を最大化しつつ、ソフト制約によりスケジュールの質を向上させます。
...
ソフト制約は「必ず守る」のではなく「なるべく守る」制約です。目的関数にペナルティ/ボーナスとして組み込まれ、**配置数を犠牲にせずに**スケジュールの質を向上させます。重みは constraint シートの `value` 列で調整可能です（1.0 未満を維持してください）。

| code | 説明 | デフォルト重み |
|---|---|---|
| `soft_spread_subject_across_days` | 同じ科目は同じ日に固まらないほうがよい | 0.1 |
| `soft_student_consecutive_slots` | 生徒のコマはなるべく連続するようにする | 0.05 |

> ソフト制約の設計詳細（目的関数の数式、重みの安全性証明、具体例、調整ガイド）は **[CONSTRAINTS.md](./CONSTRAINTS.md)** を参照してください。
...
このシステムはハード制約をすべて守った範囲で**配置数（コマ数）を最大化**し、ソフト制約で配置の質を向上させます。
